...
Notes:
       To reduce memory copy when pass data buffer to other layers, RTK_BUFFER is designed referring to linux socket buffer.
       But I still wonder its effect, since RTK_BUFFER is much bigger than original data buffer.
       RTK_BUFFER will reduce its member if it would not reach what i had expected.
--*/

/**
\file     bt_skbuff.h
\brief   Data buffer managerment through whole bluetooth stack.
*/

#ifndef _INC_SKBUFF_H
#define _INC_SKBUFF_H
#include "bt_list.h"
