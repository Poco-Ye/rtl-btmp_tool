...
#if (HCI_DBG == TRUE)
#define HCIDBG(param, ...) {SYSLOGD(param, ## __VA_ARGS__);}
#else
#define HCIDBG(param, ...) {}
#endif
...
static HC_BT_HDR *acl_rx_frame_buffer_alloc (void)
{
    ...
    if (pkt_type == ACL_RX_PKT_START)       /*** START PACKET ***/
    {
        ...
        if (p_return_buf)
        {
            SYSLOGW("H4 - dropping incomplete ACL frame");
    ...
    while (TRUE)
    {
        ...
        case H4_RX_MSGTYPE_ST:
            ...
            if ((byte < H4_TYPE_ACL_DATA) || (byte > H4_TYPE_EVENT))
            {
                ...
                /* Drop this byte */
                SYSLOGE("[h4] Unknown HCI message type drop this byte 0x%x", byte);
                break;
            }
        ...
        case H4_RX_LEN_ST:
            ...
            if (p_cb->rcv_len == 0)
            {
                ...
                if (p_cb->p_rcv_msg == NULL)
                {
                    ...
                    SYSLOGE("H4: Unable to acquire buffer for incoming HCI message.");
