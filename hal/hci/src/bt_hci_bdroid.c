...
#if (BTHC_DBG == TRUE)
#define BTHCDBG(param, ...) {SYSLOGD(param, ## __VA_ARGS__);}
#else
        ...
        bt_hci_if_t hci_if, const char *dev_node)
{
    ...
    if (hci_if == BT_HCI_IF_UART5) {
        ...
        SYSLOGE("Init failed with no HCI interface specified!");
        return BT_HC_STATUS_FAIL;
    }
...
static void *bt_hc_worker_thread(void *arg)
{
    ...
    SYSLOGI("bt_hc_worker_thread started");
    ...
    SYSLOGI("bt_hc_worker_thread exiting");
